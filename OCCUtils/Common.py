...
from OCP.TopoDS import TopoDS_Edge, TopoDS_Shape, TopoDS_Wire, TopoDS_Vertex
...
def common_vertex(edg1, edg2):
    from OCP.TopExp import topexp_CommonVertex
...
def intersection_from_three_planes(planeA, planeB, planeC):
    ...
    @param show:
    """
    from OCP.IntAna import IntAna_Int3Pln
...
class GpropsFromShape(object):
    ...
    def volume(self):
        ...
        prop = GProp_GProps()
        BRepGProp.VolumeProperties_s(self.shape, prop, self.tolerance)
        return prop

    def surface(self):
        """returns the area of a surface"""
        prop = GProp_GProps()
        BRepGProp.SurfaceProperties_s(self.shape, prop, self.tolerance)
        return prop

    def linear(self):
        """returns the length of a wire or edge"""
        prop = GProp_GProps()
        BRepGProp.LinearProperties_s(self.shape, prop)
        return prop
...
def project_point_on_plane(plane, point):
    ...
    @param point: gp_Pnt
    """
    from OCP.ProjLib import projlib_Project
