...
## Session 39: manipulator spurious-capture bug -- confirmed shared
with Basicad, one concrete fix, one targeted improvement, architecture
question left open
...
Kodacad-specific porting error -- it's a real limitation of the
underlying manual-detection pattern both codebases share, not
something introduced in this port.
...
Fixed with the confirmed-correct import.

### Targeted improvement: DetectedInteractive() as a second gate
...
the same pattern Session 23 used (ported from Basicad, which is very
likely why the bug is shared). Added `context.DetectedInteractive()
== self._manipulator` as an additional, more specific check alongside
`HasActiveMode()`, confirming the object actually detected under the
cursor is specifically the manipulator, not just trusting
`HasActiveMode()` in isolation.

### Left genuinely open: the architectural question

The SAME forum thread contains a more significant claim: "I use
AIS_ViewController with AIS_Manipulator, so no need to call
AIS_Manipulator::StartTransform... EventManager is derived from
AIS_ViewController" -- suggesting the robust fix is letting
AIS_ViewController drive the manipulator directly, rather than
Session 23's approach of manually calling StartTransform/Transform/
StopTransform in parallel. NOT attempted this session: no confirmed,
concrete example of exactly how that internal wiring works was found,
and guessing through an architecture change on that gap risks
introducing new, worse bugs -- exactly the pattern this project has
been burned by before (WriteNames, the Session 29 native-rebuild
attempt). If the DetectedInteractive() fix isn't sufficient, this
remains the next thing to investigate properly, with a real working
example in hand before attempting it.
...
correctly redirected the investigation toward the shared underlying
pattern instead -- exactly the kind of test that should be reached
for earlier when a ported feature misbehaves, the same way `minimal_
repro.py` isolating Kodacad-specific behavior from general OCCT
behavior did earlier in this project.

## Session 39 (cont'd): deferred -- crash on empty-space drag, no
diagnostic output

Doug's test confirmed the `DetectedInteractive()` fix worked for its
intended purpose (dragging in empty space right after entering
Dynamic mode no longer triggers spurious manipulator capture) -- but
surfaced two further problems:

1. The drag also doesn't fall through to normal camera orbit the way
   it should when nothing is captured (expected: orbit; actual:
   nothing happens).
2. The application crashed a few seconds later, with NOTHING printed
   to the terminal -- consistent with a native-level crash (likely in
   the OCCT/OpenGL layer itself, not a catchable Python exception),
   which is a materially harder class of problem than anything fixed
   by reading a Python traceback so far in this project.

**Decision: documented and deferred, not chased further.** Doug's
call, and the right one -- Dynamic mode's core value (drag + Nudge)
remains real and proven (the lathe-to-plate mating), and 2 Points and
Mate/Align are fully solid, unaffected alternatives for anything
where Dynamic's residual instability is a concern. Whether Session
39's changes are the actual cause of the crash, or just the first
test scenario that happened to trigger a pre-existing native-level
issue, is genuinely unknown -- not enough evidence either way to
attribute cause responsibly, and chasing a no-output native crash
would need a different, heavier kind of debugging (core dumps, gdb)
than anything this investigation has needed so far.

### Status of Dynamic, honestly stated

Works and is genuinely useful for the common case (drag roughly into
place, refine with Nudge). Has a known, unresolved edge case around
empty-space drags immediately after entering the mode, which can
progress to an application crash with no diagnostic output. Not
blocking -- 2 Points and Mate/Align cover positioning fully on their
own -- but real, and worth knowing about before relying on Dynamic
for anything where a crash mid-session would be costly.

### Lesson for future development

**Recognizing when a bug has crossed from "debuggable with the tools
at hand" into "needs different, heavier tools" is itself a useful
diagnosis, even without a fix.** A crash with zero output is
qualitatively different from every other bug this project has solved
-- there's been a stack trace, a print statement, or a file to grep
in every prior case. Naming that difference plainly, rather than
attempting more print-and-pray iterations against a problem that
doesn't produce output to read, is the responsible way to hand this
off to a future session (or a future debugging approach) rather than
