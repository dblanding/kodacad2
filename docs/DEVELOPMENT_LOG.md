...
## Known, deliberately deferred: Shell still fails on the STEP-imported OCC Bottle

Re-running the exact same routine (fillet, then Shell) that now works cleanly on a natively-extruded cup, but on the pre-shelled OCC Bottle solid loaded via Import STEP, still fails with the identical error message. Diagnosed far enough to confirm this is a genuinely different, third problem, not a gap in this session's own fix: `_needs_analytic_workaround` returns **False** for this imported bottle -- the Session 60/91 surrogate-mapping machinery never activates at all, confirmed directly (no uid mismatch, no map-build failure -- the diagnostic print showed the decision itself came back negative). Whatever is actually failing here is completely unrelated to anything fixed this session.

Not investigated further this session, deliberately -- a real, separate problem deserves its own session rather than scope-creeping into this one. STEP-imported geometry carrying its own topological quirks from the exporting process (different tolerances, different patch boundaries around the neck, from however the file was originally written) is a plausible starting hypothesis for next time, not a confirmed one.
