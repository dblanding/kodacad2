...
# face_normal implemented inline (replaces OCCUtils.Construct.face_normal)
from OCP.BRep import BRep_Tool
from OCP.GeomLProp import GeomLProp_SLProps
from OCP.TopAbs import TopAbs_REVERSED

def face_normal(face):
    """Return gp_Dir of face normal at its centre of mass."""
    from OCP.BRepGProp import BRepGProp
    from OCP.GProp import GProp_GProps
    props = GProp_GProps()
    BRepGProp.SurfaceProperties_s(face, props)
    uv_mid = props.CentreOfMass()
    surface = BRep_Tool.Surface_s(face)
    from OCP.GeomAPI import GeomAPI_ProjectPointOnSurf
    proj = GeomAPI_ProjectPointOnSurf(uv_mid, surface)
    u, v = proj.LowerDistanceParameters()
    props2 = GeomLProp_SLProps(surface, u, v, 1, 1e-6)
    normal = props2.Normal()
